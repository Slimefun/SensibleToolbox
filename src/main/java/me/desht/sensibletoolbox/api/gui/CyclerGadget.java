...
public abstract class CyclerGadget<T extends Enum<T>> extends ClickableGadget {
     ...
     */
    public void add(T what, ChatColor colour, MaterialData texture, String... lore) {
        if (currentValue == null) {
            currentValue = what;
            stacks = new ItemStack[what.getClass().getEnumConstants().length];
        }
        stacks[what.ordinal()] =  makeTexture(what, texture, colour, lore);
    }
    ...
    public ItemStack getTexture() {
        if (stacks[currentValue.ordinal()] == null) {
            stacks[currentValue.ordinal()] = makeTexture(currentValue, new MaterialData(Material.STONE), ChatColor.GRAY);
        }
